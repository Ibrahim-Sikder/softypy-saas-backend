...
const createPurchaseReturn = async (tenantDomain: string, payload: TPurchaseReturn) => {
  ...
  try {
    ...
    const supplier = await Supplier.findById(payload.supplier).session(session);
    if (supplier) {
      supplier.totalDue -= payload.totalReturnAmount;
